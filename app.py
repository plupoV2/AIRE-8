...
/* ===== AIRE Pro Terminal — High Contrast Clean Theme ===== */
:root{
  --bg: #f6f8fb;
  --panel: #ffffff;
  --panel2: #f9fafb;
...
.card h1,.card h2,.card h3,.card h4{ color: var(--text) !important; }
...
class PropertyData:
    ...
    job_diversity_index: Optional[float] = None
...
def _irr(cashflows: list[float]) -> float | None:
    """Robust IRR without numpy.irr (which is removed in NumPy 2.x).

    Returns periodic IRR (annual, since our cashflows are annual).
    """
    try:
        if not cashflows:
            return None
        ...
        # If still no sign change, do a coarse scan
        if f_lo * f_hi > 0:
            grid = [-0.95, -0.5, -0.2, 0.0, 0.1, 0.25, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0, 40.0]
